...
char* codegen_native_generate_asm(IRModule *module, NativeTarget target) {
    ...
    if (target == NATIVE_TARGET_X86_64 || target == NATIVE_TARGET_X86_32) {
        /* Delegate to the stack-machine x64 code generator */
        FILE *tmp = tmpfile();
        if (!tmp) {
            fprintf(stderr, "Error: Failed to create temporary file for code generation\n");
            return NULL;
        }
        
        X64Context *ctx = x64_context_create(tmp);
        if (!ctx) {
            fclose(tmp);
            return NULL;
        }
        
        x64_generate_program(ctx, module);
        x64_context_free(ctx);
        
        /* Read back the generated assembly as a string */
    ...
    /* ARM64 fallback - use the old register-based codegen */
